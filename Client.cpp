...
void Client_Pers_Fizic::Comanda_Produs(std::shared_ptr<Produs> p, int cantitate_, array<std::shared_ptr<Angajat>, 2> a,
    ...
    if (cantitate_ > p->get_Cantiate()) {
        ...
        Reducere_Comadna_Produs(p, cantitate_);
    }
}

void Client_Pers_Fizic::Reducere_Comadna_Produs(std::shared_ptr<Produs> p, int cantitate_) {
    if (Vechime_Client > 3 && Vechime_Client <= 5 && Numar_Comenzi > 10 && Numar_Comenzi < 30) {
        std::cout << "Aveti o reducere de 10% pentru ca sunteti client fidel de nivelul 1!";
        std::cout << "Totalul de plata este: " << 0.9 * (p->get_Pret() * float(cantitate_)) << endl;
        Numar_Comenzi++;
    } else if (Numar_Comenzi >= 30 && Numar_Comenzi <= 50) {
        std::cout << "Aveti o reducere de 20% pentru ca sunteti fidel de niveul 2!";
...
Client_Pers_Juridic::Comanda_Produs(std::shared_ptr<Produs> p, int cantitate_, array<std::shared_ptr<Angajat>, 2> a,
    ...
    if (cantitate_ < 30) {
        ...
        switch (optiune) {
            ...
            default:
                break;
                //Sa ma gandesc la niste exceptii pe care sa le pot pune in Comanda_Produs
        }
    } else {
        if (cantitate_ > p->get_Cantiate()) {

            std::cout << "Nu avem cantitatea dorita, vom plasa comanda la cofetar!" << endl;
            a[1]->Lucreaza1(p, cantitate_ - p->get_Cantiate() + 1);//aici cofetarul mai prepara produse
...
void Client_Pers_Juridic::Reducere_Comadna_Produs(std::shared_ptr<Produs> p, int cantitate_) {
